...
const SelectDrink: React.FC = () => {
  ...
  const fetchDrinks = async () => {
    try {
      ...
      if (data.drinks) {
        setDrinks(data.drinks.slice(0, 20)); // Limit to 20 drinks for performance
      }
    } catch (error) {
  ...
  const toggleDrinkSelection = (drink: DrinkAPI) => {
    ...
    if (exists) {
      setSelectedDrinks(selectedDrinks.filter((d) => d.idDrink !== drink.idDrink));
    } else {
                  ...
                  <div className="mt-2">
                    <label className="block text-sm">
                      ...
                      <input
                        ...
                        min="1"
                        value={
                          selectedDrinks.find((d) => d.idDrink === drink.idDrink)
                            ?.quantity || 1
                        }
                        onChange={(e) =>
                          updateQuantity(
