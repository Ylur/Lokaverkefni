...
router.post('/', [
    ...
    body('dishes.*.quantity').isInt({ min: 1 }).withMessage('Dish quantity must be at least 1.'),
    ...
    body('drinks.*.quantity').isInt({ min: 1 }).withMessage('Drink quantity must be at least 1.'),
...
router.put('/:id', [
    ...
    body('dishes.*.idMeal').optional().notEmpty().withMessage('Dish ID is required.'),
    body('dishes.*.strMeal').optional().notEmpty().withMessage('Dish name is required.'),
    body('dishes.*.quantity').optional().isInt({ min: 1 }).withMessage('Dish quantity must be at least 1.'),
    body('drinks').optional().isArray().withMessage('Drinks must be an array.'),
    body('drinks.*.idDrink').optional().notEmpty().withMessage('Drink ID is required.'),
    body('drinks.*.strDrink').optional().notEmpty().withMessage('Drink name is required.'),
    body('drinks.*.quantity').optional().isInt({ min: 1 }).withMessage('Drink quantity must be at least 1.'),
    body('total').optional().isFloat({ gt: 0 }).withMessage('Total must be a positive number.')
